...
- **旧 `lead_submit` のキーイベント指定を解除**

## 7. 移行の完了条件

新イベントで7日分のデータが取れ、`lead_submit` と件数が一致していることを確認したら：

1. GA4から `lead_submit` のキーイベント指定を解除（済んでいなければ）
2. コード側の併走を削除
   `src/pages/diagnostic/start/index.astro` の
